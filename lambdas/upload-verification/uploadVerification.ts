...
/** POST /teachers/{teacherId}/verification-url */
export const handler = async (
  ...
  try {
    ...
    if (typeof fileType !== "string" || !ALLOWED_TYPES.includes(fileType as (typeof ALLOWED_TYPES)[number])) {
      return jsonResponse(400, {
        success: false,
        error: "fileType must be one of: application/pdf, image/jpeg, image/png",
      } satisfies ApiResponse<never>);
    }

    const typedFileType = fileType as (typeof ALLOWED_TYPES)[number];
    const bucket = requireEnv("S3_PROFILE_PHOTOS_BUCKET");
    const extension = EXTENSIONS[typedFileType];
    const key = `verification-docs/${teacherId}/${Date.now()}.${extension}`;
    ...
    return jsonResponse(200, {
      ...
      data: {
        uploadUrl,
        docUrl: `https://${bucket}.s3.ap-southeast-1.amazonaws.com/${key}`,
        expiresIn,
      },
    } satisfies ApiResponse<{ uploadUrl: string; docUrl: string; expiresIn: number }>);
  } catch (error) {
